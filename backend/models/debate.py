...
def next_round(req: NextRoundRequest):
    ...
    # This call generates exactly ONE new round.
    next_round_number = debate.current_round + 1

    # Carry the previous round forward so the new round is a real rebuttal,
    # not a fresh restart.
    last = debate.rounds[-1] if debate.rounds else None
    ...
    initial: DebateState = {
        ...
        "prev_pro_argument": last.pro_argument if last else "",
        "prev_con_argument": last.con_argument if last else "",
        "pro_argument": "",
        "con_argument": "",
        "judge_feedback": "",
        "score_pro": 0.0,
    ...
    round_data = DebateRound(
        round_number=next_round_number,            # numbered once, no double-increment
    ...
    except DebateCompletedError as e:
